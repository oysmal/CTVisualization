...
vec4 sampleAs3DTexture( vec3 texCoord ) {
  ...
  float zSliceNumber1 = floor(texCoord.z  * 63.0);

  //As we use trilinear we go the next Z slice.
  float zSliceNumber2 = min( zSliceNumber1 + 1.0, 63.0); //Clamp to 255
  ...
  texCoordSlice1.y += floor((63.0 - zSliceNumber1) / 8.0) / 8.0;
  texCoordSlice2.x += (mod(zSliceNumber2, 8.0 ) / 8.0);
  texCoordSlice2.y += floor((63.0 - zSliceNumber2) / 8.0) / 8.0;
